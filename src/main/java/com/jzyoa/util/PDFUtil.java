...
public class PDFUtil implements ApplicationContextAware {
	...
	public static void pdfout(Map<String, Object> data, String templatePath, String newPDFPath) {
		...
		PdfStamper stamper;
		try {
			String fontStr = new Object() {
			}.getClass().getClassLoader().getResource("songti.ttf").getPath();
	...
	public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
		...
		if (PDFUtil.applicationContext == null) {
			...
			try {
				...
				}.getClass().getClassLoader().getResource("songti.ttf").getPath(), BaseFont.IDENTITY_H,
						BaseFont.EMBEDDED);
				...
				for (TbDictionary dictionary : testTypeList) {
					pdfTemplatePathMap.put(dictionary.getDictId(),
							basePath + dictionary.getDictMark() + "_template.pdf");
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
